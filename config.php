<?php
    ...
    $config['google_secret'] = '6Le5cAkUAAAAANmrzq6hbhPyyqATrPXSBlV9PhzN';
    $config['google_sitekey'] = '6Le5cAkUAAAAANXZGm7WPRd9GO038hea3qgaKoNv';
	...
	$config['sqlUser'] = 'bshift';

	// phpmyadmin password for OT server:
	$config['sqlPassword'] = 'bxlFCq3nzYSI2WlG';

	// The database name to connect to. (This is usually same as username).
	$config['sqlDatabase'] = 'blueshift';
	...
	function newsTickerDate($time = false, $format = false, $adjust = true) {
		if ($time === false) $time = time();
		// Date string representation
		$date = "d F Y"; // 15 July 2013 (13:50)
		if ($adjust) $adjust = (1 * 3600); // Adjust to fit your timezone.
		else $adjust = 0;
		if ($format) return date($date, $time+$adjust);
		else return $time+$adjust;
