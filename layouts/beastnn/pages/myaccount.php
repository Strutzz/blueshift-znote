...
<div class="content bg-image overflow-hidden" style="background-image: url('layouts/beastn/assets/img/photos/photo3@2x.jpg');">
    <div class="push-50-t push-15">
        <h1 class="h2 text-white animated zoomIn">My Account</h1>
			...
			case 'change_comment':
				...
				if (user_character_account_id($char_name) === $session_user_id) {
                    ...
                    <div class="block-header bg-primary">
                        <h3 class="block-title">Change comment on <?php echo $char_name; ?></h3>
                    </div>
                    <div class="block-content">

					<form class="form-horizontal push-10-t push-10" action="" method="post">
                        <input name="action" type="hidden" value="update_comment">
                        <input name ="selected_character" type="hidden" value="<?php echo $char_name; ?>" readonly="readonly">

                        <div class="form-group">
                            <div class="col-xs-12">
                                <label for="login2-username">Comment</label>
                                <textarea class="form-control" name="comment" cols="70" rows="10"><?php echo $comment_data['comment']; ?></textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-xs-12">
                                <?php Token::create(); ?>
                                <button class="btn btn-sm btn-primary" type="submit"><i class="fa fa-arrow-right push-5-r"></i> Change Comment</button>
                            </div>
                        </div>
					</form>
...
if ($render_page) {
    ...
    <!-- Page Content -->
            ...
            <div class="block block-themed">
                <div class="block-header bg-primary">
                    <h3 class="block-title">account overview</h3>
                </div>
                <div class="block-content">
                    ...
                    <h4>Character List: <?php echo $char_count; ?> characters.</h4>
                    <?php
                    // Echo character list!
                    ...
                    if ($char_array) {
                        ...
                        <table id="myaccountTable" class="table table-striped table-hover">
                            ...
                            foreach ($char_array as $value) {
                                ...
                                echo '<tr>';
                                echo '<td><a href="'.url('characterprofile&name='.$value['name']).'">'. $value['name'] .'</a></td><td>'. $value['level'] .'</td><td>'. $value['vocation'] .'</td><td>'. $value['town_id'] .'</td><td>'. $value['lastlogin'] .'</td><td>'. $value['online'] .'</td><td>'. hide_char_to_name(user_character_hide($value['name'])) .'</td>';
                                echo '</tr>';
                        ...
                        echo '<p>You don\'t have any characters. Why don\'t you <a href="/?subtopic=createcharacter">create one</a>?</p>';
                    }
                    ?>
                </div>
            </div>
